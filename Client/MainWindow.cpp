...
bool CMainWindow::OnCreate(const HINSTANCE& hInstance, const HWND& hMainWnd)
{
...
{
	while (m_sptrSubWindows.size()) {
		auto subwindow = m_sptrSubWindows.top();
		m_sptrSubWindows.pop();
...
void CMainWindow::CreateSwapChain()
{
	...
	dxgiSwapChainDesc1.Flags = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH | DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING;
	...
	DXGI_SWAP_CHAIN_FULLSCREEN_DESC dxgiSwapChainFullDesc;
	ZeroMemory(&dxgiSwapChainFullDesc, sizeof(dxgiSwapChainFullDesc));
	dxgiSwapChainFullDesc.RefreshRate.Numerator = 0;
	dxgiSwapChainFullDesc.RefreshRate.Denominator = 0;
	dxgiSwapChainFullDesc.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;
	dxgiSwapChainFullDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
	dxgiSwapChainFullDesc.Windowed = TRUE;

	HRESULT hResult = m_pdxgiFactory->CreateSwapChainForHwnd(m_pd3dCommandQueue, m_hWnd, &dxgiSwapChainDesc1, &dxgiSwapChainFullDesc, NULL, (IDXGISwapChain1**)&m_pdxgiSwapChain);
	hResult = m_pdxgiFactory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER);
	m_nSwapChainBufferIndex = m_pdxgiSwapChain->GetCurrentBackBufferIndex();
}

void CMainWindow::CreateRtvAndDsvDescriptorHeaps()
{
...
void CMainWindow::CreateDirect3DDevice()
{
	HRESULT hResult;
...
void CMainWindow::CreateCommandQueueAndList()
{
...
void CMainWindow::CreateDepthStencilView()
{
...
void CMainWindow::BuildObjects()
{
	...
	m_pScene = new CScene(m_nWndClientWidth, m_nWndClientHeight);
...
(D3D12_RESOURCE_STATE_PRESENT)에서 렌더 타겟 상태(D3D12_RESOURCE_STATE_RENDER_TARGET)로 바뀔 것이다.*/
	...
	BOOL b_fullscreen;
	m_pdxgiSwapChain->GetFullscreenState(&b_fullscreen, NULL);
	UINT presentFlags = 0;
	if (!b_fullscreen)
		presentFlags |= DXGI_PRESENT_ALLOW_TEARING;
	m_pdxgiSwapChain->Present(0, presentFlags);
	// 티어링을 허가하면 프레임이 올라간다
...
void CMainWindow::OnProcessingMouseMessage(const HWND& hWnd, const HINSTANCE& hInst, const UINT& nMessageID, const WPARAM& wParam, const LPARAM& lParam)
{
	switch (nMessageID)
	{
	case WM_LBUTTONDOWN:
...
void CMainWindow::OnProcessingKeyboardMessage(const HWND& hWnd, const HINSTANCE& hInst, const UINT& nMessageID, const WPARAM& wParam, const LPARAM& lParam)
{
	switch (nMessageID)
	{
	case WM_KEYUP:
		...
		case VK_RETURN:
		{
			CStreamDeckWindow* StreamDeckPopup = new CStreamDeckWindow(hInst, 100, 100, 200, 200);
			StreamDeckPopup->CreateSubWindow();
			m_sptrSubWindows.emplace(StreamDeckPopup);
		}
		break;
		case VK_F8:
			break;
...
LRESULT CMainWindow::OnProcessingWindowMessage(const HWND& hWnd, const HINSTANCE& hInst, const UINT& nMessageID, const WPARAM& wParam, const LPARAM& lParam)
{
	switch (nMessageID)
	{
	case WM_SIZE:
	...
	case WM_RBUTTONUP:
	case WM_MOUSEMOVE:
		OnProcessingMouseMessage(hWnd, hInst, nMessageID, wParam, lParam);
		break;
	case WM_KEYDOWN:
	case WM_KEYUP:
		OnProcessingKeyboardMessage(hWnd, hInst, nMessageID, wParam, lParam);
		break;
	}
	return 0;
}

void CMainWindow::ChangeSwapChainState()
{
	WaitForGpuComplete();
	...
	m_pdxgiSwapChain->ResizeBuffers(m_nSwapChainBuffers, m_nWndClientWidth, m_nWndClientHeight, dxgiSwapChainDesc1.Format, dxgiSwapChainDesc1.Flags);
	m_nSwapChainBufferIndex = m_pdxgiSwapChain->GetCurrentBackBufferIndex();
	CreateRenderTargetViews();
	CreateDepthStencilView();
}
