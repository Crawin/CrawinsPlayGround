...
CCubeMeshDiffusedIndexed::CCubeMeshDiffusedIndexed(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth)
{
	//직육면체는 꼭지점(정점)이 8개이다. 
	m_nVertices = 8;
	m_nStride = sizeof(CDiffusedVertex);
...
void CCubeMeshDiffusedIndexed::ReleaseUploadBuffers()
{
...
void CCubeMeshDiffusedIndexed::Render(ID3D12GraphicsCommandList* pd3dCommandList)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
	pd3dCommandList->IASetVertexBuffers(m_nSlot, 1, &m_d3dVertexBufferView);
...
void CCubeMeshDiffusedIndexed::Render(ID3D12GraphicsCommandList* pd3dCommandList,UINT nInstances, D3D12_VERTEX_BUFFER_VIEW d3dInstancingBufferView)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
