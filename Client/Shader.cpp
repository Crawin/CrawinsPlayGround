...
void CSkyboxShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dRootSignature)
{
	if (m_nPipelineStates == 0) {
		m_nPipelineStates = 1;
		m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];
...
D3D12_INPUT_LAYOUT_DESC CSkyboxShader::CreateInputLayout()
{
	...
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
	pd3dInputElementDescs[0] = { "WORLDMATRIX", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(float) * 4 * 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[1] = { "WORLDMATRIX", 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(float)*4 * 1, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0};
	pd3dInputElementDescs[2] = { "WORLDMATRIX", 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(float) * 4*2, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[3] = { "WORLDMATRIX", 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, sizeof(float) * 4 * 3, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[4] = { "PADDING", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, sizeof(float) * 4 * 4, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VS_SKYBOX", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CSkyboxShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PS_SKYBOX", "ps_5_1", ppd3dShaderBlob));

}

D3D12_SHADER_BYTECODE CSkyboxShader::CreateGeomertyShader(ID3DBlob** ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "GS_SKYBOX", "gs_5_1", ppd3dShaderBlob));
...
void CSkyboxShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	m_nObjects = 1;
	m_ppObjects = new CObject * [m_nObjects];

	m_ppObjects[0] = new CSkybox;
	GS_CB data;
	XMStoreFloat4x4(&data.worldTransform, ::XMMatrixTranspose(XMLoadFloat4x4(&m_ppObjects[0]->m_xmf4x4World)));
	data.padding = XMFLOAT3(100, 100, 100);
	m_pd3dVertexBuffer = CreateBufferResource(pd3dDevice, pd3dCommandList, &data, sizeof(GS_CB) * m_nObjects, D3D12_HEAP_TYPE_DEFAULT,
		D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dVertexUploadBuffer);
	m_d3dVertexBufferView.BufferLocation = m_pd3dVertexBuffer->GetGPUVirtualAddress();
	m_d3dVertexBufferView.SizeInBytes = sizeof(GS_CB) * m_nObjects;
	m_d3dVertexBufferView.StrideInBytes = sizeof(GS_CB);
	CTexture* ppTextures[1];
	ppTextures[0] = new CTexture;
	ppTextures[0]->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"MilkyWayPanorama8K.dds", 0, 0);
	m_pTexture = ppTextures[0];
...
void CSkyboxShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (m_pTexture)
		m_pTexture->Render(pd3dCommandList);
	pd3dCommandList->DrawInstanced(m_nObjects, 1, 0, 0);
}
